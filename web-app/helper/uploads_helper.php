...
//Upload Helper to multiple file upload class
function file_object($argument){
	
	$uploaded_files = $_FILES[$argument]??NULL;

	if(is_null($uploaded_files)){
		exit('Invalid '.$argument.' Field Name ');
	}else{
		@$count = @count(@$uploaded_files['name']);
		if($count==1){
			return $uploaded_files;
		}else{
			// Return multiple files Object
			return $uploaded_files;
			// Return multiple files Object
		}
	}
