...
<form class="form-horizontal" action="<?php echo basename($_SERVER['PHP_SELF']);?>" method="post">
	...
	<div class="form-group">
		...
		<div class="col-xs-3">
        <input type="text" class="form-control" name="db" id="db" placeholder="" value="bulk_database">
		</div>
    </div>
	...
	<div class="form-group">
		...
		<div class="col-xs-3">
        <input type="name" class="form-control" name="table" id="table" value="master_table">
		</div>
    </div>
	<div class="form-group">
        ...
        <select class="form-control" name="csv" id="csv">
			<?php 
				...
				$tmp_arr=scandir(__DIR__."/tmp/");
				unset($tmp_arr[0],$tmp_arr[1]);
				foreach($tmp_arr as $index => $file){
